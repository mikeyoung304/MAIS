...
/**
 * Hook to auto-redirect to Build Mode when tenant reaches BUILDING status
 *
 * Features:
 ...
 * @param currentStatus - Current onboarding status from useOnboardingState
...
export function useBuildModeRedirect(
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Redirect to Build Mode when tenant reaches BUILDING status (once only)
      if (currentStatus === 'BUILDING' && !hasRedirected) {
  ...
  }, [pathname, tenantId, currentStatus, isLoading, router]);
}
