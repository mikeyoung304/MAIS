...
function Select({
  ...
  return (
    <select
      className={`flex h-10 w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm focus:border-sage focus:outline-none focus:ring-2 focus:ring-sage/20 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
      {...props}
    >
      {children}
    </select>
