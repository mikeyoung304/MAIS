...
39. **Trust tier mismatch between tool definition and createProposal:** Tool definitions may say `trustTier: 'T1'` but pass `'T2'` to `createProposal()`. The orchestrator checks the PROPOSAL's tier, not the tool definition. Always ensure both match.
40. **TanStack Query staleTime blocking real-time updates:** For agent-driven or real-time features, `staleTime` must be `0`. Even with `invalidateQueries()`, data won't refetch if considered "fresh". Also add `refetchType: 'active'` to force refetch.
41. **Race condition on cache invalidation after DB write:** PostgreSQL transactions + connection pooling can cause refetch to hit DB before write is visible. Add 100ms delay before `invalidateQueries()` for real-time features.
...
- **[code-review-708-717-prevention](docs/solutions/patterns/CODE_REVIEW_708_717_PREVENTION_STRATEGIES.md)** - 10 patterns from parallel code review (#708-717): TOCTOU races with advisory locks, unsafe type assertions, premature optimization, component/service duplication, React 19 ref types. Covers P1 fix (TOCTOU) + P2 fixes (5 patterns) + P3 deferred (4 patterns).
  - Quick reference: [CODE_REVIEW_708_717_QUICK_REFERENCE.md](docs/solutions/patterns/CODE_REVIEW_708_717_QUICK_REFERENCE.md) - Print & pin (2 min read)
- **[paintbrush-effect-trust-tier-stale-cache](docs/solutions/agent-issues/paintbrush-effect-trust-tier-stale-cache-MAIS-20260111.md)** - Trust tier mismatch (tool says T1, createProposal passes T2) + TanStack Query staleTime blocking real-time updates + race condition with connection pooling. Triple-layer fix for agent-driven preview updates.

**Key insight from Code Review #708-717 (Commit 02cde7e8):** Specialized reviewers find non-overlapping issues - data-integrity-guardian caught TOCTOU race in maxPerDay check that other reviewers missed. Solution: Wrap check-then-act patterns in `$transaction` with advisory lock (hash tenant+service+date). Type assertions require type predicates with structural validation BEFORE cast. Component duplication (same styling differences) solved with variant prop pattern + variantStyles object. Service logic duplication extracted to private methods callable from all public paths. Premature optimization prevented by ALWAYS measuring first - documentation states "measured latency: 60ms = 45ms DB + 12ms validation, so caching adds complexity without benefit". React ref type mismatch (RefObject<T | null> vs JSX LegacyRef<T>) solved by casting useRef<T>(null) as React.RefObject<T>.
...
**Key insight from Paintbrush Effect Fix (Commits ecd60e5f, 1ddcdc99):** Double-layered bugs require all layers to be fixed. Bug was hard to find because each layer looked correct in isolation: (1) Tool definitions said `trustTier: 'T1'` but `createProposal()` passed `'T2'` - orchestrator checks proposal tier, not tool definition, so proposals stayed PENDING; (2) `staleTime: 30_000` meant TanStack Query considered data "fresh" for 30s even after invalidation; (3) Connection pooling could cause refetch before write was visible. Fix: Align trust tiers, set `staleTime: 0`, add `refetchType: 'active'`, add 100ms delay. Always trace the full flow: **Tool → Proposal → Executor → DB → Cache → Refetch → UI**.
