...
function validateExecutorRegistry(): void {
  ...
  const missing = REQUIRED_EXECUTOR_TOOLS.filter((tool) => !registeredTools.includes(tool));
...
export async function getTenantInfo(
  ...
  return tenant
    ? {
        ...
        timezone: options?.includeTimezone ? (tenant as any).timezone : undefined,
      }
    : null;
}
```
...
test('delete operation respects tenantId scope', async () => {
  ...
  await expect(executeDeleteService(tenantA, serviceB.id)).rejects.toThrow(ResourceNotFoundError);
