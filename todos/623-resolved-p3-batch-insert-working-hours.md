...
await prisma.$transaction(async (tx) => {
  ...
  for (const entry of workingHours) {
    if (entry.isActive) {
      await tx.availabilityRule.create({  // Sequential inserts - N round trips
        data: { tenantId, serviceId: null, dayOfWeek: entry.dayOfWeek, startTime: entry.startTime, endTime: entry.endTime }
      });
    }
  }
});
