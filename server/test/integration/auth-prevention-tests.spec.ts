 ...
 *
 * PREREQUISITES:
 * - DATABASE_URL: Required for real database access
 * - GOOGLE_VERTEX_PROJECT: Required for agent routes initialization
 *
 * These tests are skipped in CI environments where prerequisites aren't available.
 */
...
// Check prerequisites for these tests
// - DATABASE_URL: Required for buildContainer with real adapters
// - GOOGLE_VERTEX_PROJECT: Required for createApp (agent routes)
const hasPrerequisites = Boolean(process.env.DATABASE_URL && process.env.GOOGLE_VERTEX_PROJECT);

// Wrap entire test suite in skipIf to prevent beforeAll from running at all
describe.skipIf(!hasPrerequisites)('Auth Prevention Tests', () => {
