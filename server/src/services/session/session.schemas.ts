...
/**
 * Generate a unique idempotency key for messages without one.
 * Format: auto-{timestamp}-{random} to prevent collisions
 */
function generateIdempotencyKey(): string {
  return `auto-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
}
 ...
 *
 * IMPORTANT: idempotencyKey is auto-generated if not provided.
 * This ensures the unique constraint on (sessionId, idempotencyKey) always provides
 * protection - PostgreSQL treats NULL as distinct values, so without this default,
 * duplicate messages with NULL idempotencyKey could be inserted.
 */
...
/**
 * Input type for CreateMessageInputSchema - what callers provide.
 * idempotencyKey is optional; will be auto-generated by the schema if not provided.
 */
export type CreateMessageInput = z.input<typeof CreateMessageInputSchema>;

/**
 * Output type for CreateMessageInputSchema - what comes out after validation.
 * idempotencyKey is always present after transform.
 */
export type CreateMessageOutput = z.output<typeof CreateMessageInputSchema>;
