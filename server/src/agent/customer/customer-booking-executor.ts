...
export function registerCustomerBookingExecutor(prisma: PrismaClient): void {
  registerCustomerProposalExecutor('create_customer_booking', async (tenantId, customerId, payload) => {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (existingBooking) {
        throw new Error('Sorry, this date is no longer available. Please choose a different date.');
      }
