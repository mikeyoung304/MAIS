...
import { generateServiceSlug, buildBookingUrl, type WorkingHoursEntry } from '@macon/contracts';
...
export function registerBookingLinkExecutors(prisma: PrismaClient): void {
  ...
  registerProposalExecutor('manage_bookable_service', async (tenantId, payload) => {
    ...
    if ('updates' in payload) {
      ...
      if (updates.minNoticeMinutes !== undefined)
        updateData.minNoticeMinutes = updates.minNoticeMinutes;
  ...
  registerProposalExecutor('manage_working_hours', async (tenantId, payload) => {
    ...
    logger.info(
      {
        tenantId,
        activeDays: workingHours.filter((h) => h.isActive).length,
  ...
  registerProposalExecutor('manage_date_overrides', async (tenantId, payload) => {
    ...
    if ('startDate' in payload && 'endDate' in payload) {
      ...
      const { date, available, reason } = typedPayload;
      ...
      logger.info({ tenantId, date, available, reason }, 'Date override added via executor');
